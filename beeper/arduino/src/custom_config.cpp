...
static inline void toggleConfig() {
    ...
    const uint16_t eeprom_code = (private_::is_enabled) ? eeprom_uint16_code::DISABLED : eeprom_uint16_code::ENABLED;
    ...
    switch(state) {
        ...
        case states::IGNITION_ON_COUNTING: {
            ...
            if(custom_signals::ignition_state().isOff()) {
                changeToState(button_click_count == kExpectedButtonClicks ? states::IGNITION_OFF_TOGGLE_CONFIG : states::IGNITION_OFF_IDLE);
                break;
            }
