...
public:
    ...
    inline uint8 num_bytes() const { return num_bytes_; }
    ...
    inline uint8 get_byte(uint8 index) const { return bytes_[index]; }

    // Caller should check that num_bytes < kMaxBytes;
    inline void append_byte(uint8 value) { bytes_[num_bytes_++] = value; }
