...
namespace lin_processor {
  ...
  public:
    ...
    inline uint8_t prescaling() const { return prescaling_; }

    inline uint16_t counts_per_bit() const { return counts_per_bit_; }
    inline uint16_t counts_per_half_bit() const { return counts_per_half_bit_; }
    inline uint8_t millis_per_bit() const { return millis_per_bit_; }
    inline uint8_t millis_per_half_bit() const { return millis_per_half_bit_; }
    inline uint8_t millis_until_start_bit() const { return millis_until_start_bit_; }
  ...
  static inline void setErrorFlags(Errors flags) {
    error_pin.high();
    // Non atomic when called from setup() but should be fine since ISR is not running
    // yet.
    error_flags |= flags;
    error_pin.low();
  }
  ...
  inline void StateReadData::finish() {
    ...
    // NOTE: we will reset the byte_count of the new frame buffer next time we will enter
    // data detect state. NOTE: verification of sync byte, id, checksum, etc is done
    // latter by the main code, not the ISR.
  ...
  ISR(TIMER1_CAPT_vect) {
    // Add half bit duration to value from start bit (we lost four bits for noise
    // cancelling)
