...
static const BitName kErrorBitNames[] PROGMEM = {
    {errors::FRAME_TOO_SHORT, "SHRT"}, {errors::FRAME_TOO_LONG, "LONG"}, {errors::START_BIT, "STRT"}, {errors::STOP_BIT, "STOP"},
    {errors::SYNC_BYTE, "SYNC"},       {errors::BUFFER_OVERRUN, "OVRN"}, {errors::OTHER, "OTHR"},
};
...
void setup() {
    ...
    sio::printf(F("LIN: %u, %u, %u, %u, %u, %u, %u, %u\n"), config.baud(), custom_defs::kUseLinChecksumVersion2, config.prescaler_x64(),
                config.counts_per_bit(), config.counts_per_half_bit(), config.clock_ticks_per_bit(), config.clock_ticks_per_half_bit(),
